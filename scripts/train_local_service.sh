#!/bin/bash

# This script is used for Pyramid-Flow Video Generation Training (Using Temporal Pyramid and autoregressive training)
# It enables the autoregressive video generative training with temporal pyramid
# make sure to set, NUM_FRAMES % VIDEO_SYNC_GROUP == 0; GPUS % VIDEO_SYNC_GROUP == 0

# ps -ef | grep train/train_video_music_clip.py | grep -v grep | cut -c 9-16 | xargs kill -9

GPUS=1  # The gpu number
MODEL_PATH="/mnt/bn/jiny-ttls-i18n-fr1q/models/Multimodal-Embedding/model_state_epoch_170000.th"
OUTPUT_DIR="/mnt/bn/jiny-ttls-i18n-fr1q/checkpoints/video_local_service_only_gpt"

BATCH_SIZE=32
GRAD_ACCU_STEPS=1
NUM_FRAMES=8
# ANNO_FILE="hdfs://harunasg/home/byte_data_tt_m/jinyang.leo/music_rec_data/finetune_v1_data"
# ANNO_FILE="/mnt/bn/jiny-ttls-i18n-fr1q/ttls_hive_data/finetune_v1_data_plus"
# ANNO_FILE="hdfs://harunava/user/huangchenzhe.7/LocalService/ls_model/datasets/train_cmb-LS-train"
ANNO_FILE="hdfs://harunava/user/huangchenzhe.7/LocalService/ls_model/datasets/train_gpt-LS-train"
CONFIG_PATH="/mnt/bn/jiny-ttls-i18n-fr1q/MM-Embedding/experiments/config_finetune_mmembed_v3.yaml"

# For the 768p version, make sure to add the args:  --gradient_checkpointing

torchrun --nnodes=${ARNOLD_WORKER_NUM} \
	--node_rank ${ARNOLD_ID}  --master_addr ${METIS_WORKER_0_HOST} --master_port ${METIS_WORKER_0_PORT} \
	--nproc_per_node ${ARNOLD_WORKER_GPU} \
    train/train_video_music_clip.py \
    --model_name video_local_service \
    --model_path $MODEL_PATH \
    --model_dtype bf16 \
    --use_text_feature \
    --cfg_path $CONFIG_PATH \
    --gradient_accumulation_steps $GRAD_ACCU_STEPS \
    --output_dir $OUTPUT_DIR \
    --batch_size $BATCH_SIZE \
    --num_workers 8 \
    --max_frames $NUM_FRAMES \
    --anno_file_path $ANNO_FILE \
    --lr_scheduler cosine \
    --ema_update \
    --ema_decay 0.995 \
    --ema_update_step 1 \
    --opt adamw \
    --opt_beta1 0.9 \
    --opt_beta2 0.95 \
    --seed 42 \
    --weight_decay 1e-4 \
    --clip_grad 1.0 \
    --lr 5e-5 \
    --warmup_steps 2000 \
    --epochs 30 \
    --iters_per_epoch 2000 \
    --report_to tensorboard \
    --print_freq 40 \
    --save_ckpt_freq 1

# torchrun --nproc_per_node ${GPUS} \