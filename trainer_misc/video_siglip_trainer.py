...
# dwa weights
total_task = 4
loss_keys = ['video_short_loss', 'video_long_loss', 'fusion_title_loss','lm_loss']
# total_task = 2
# loss_keys = ['short_text_loss', 'long_text_loss']
...
def train_one_epoch_for_video_siglip(
    runner,
    ...
    for step in metric_logger.log_every(range(iters_per_epoch), print_freq, header):
        ...
        for _ in range(args.gradient_accumulation_steps):
            ...
            with accelerator.accumulate(runner):
                ...
                # abandon some of the internvid samples:
                for key in samples:
                    if isinstance(samples[key], torch.Tensor):
                        if key.startswith('internvid_'):
                            internvid_batch = samples[key].shape[0]
                            samples[key] = samples[key][:internvid_batch//2]
                ...
                if use_dwa:
                    loss_avg = [accelerator.gather(ret_dict[key].unsqueeze(0)).mean() for key in loss_keys]
                    
                    weights = dwa_weights(loss_avg).to(ret_dict['loss'].device)
                    loss = 0.0
                    for i in range(total_task):
                        loss += weights[i] * ret_dict[loss_keys[i]]
                ...
                if not math.isfinite(loss_value):
                    ...
                    # print("video_text_loss: ", ret_dict['video_text_loss'])
                    # print("fused_text_loss: ", ret_dict['fused_text_loss'])
                    # print("lm_loss: ", ret_dict['lm_loss'])
            ...
            if accelerator.sync_gradients:
                ...
                # if 'nce_temperature' in ret_dict:
                #     metric_logger.update(nce_temperature=ret_dict['nce_temperature'].item())
                train_loss_dict = {}
                # train_loss = 0.0
