...
def init_distributed_mode(args, init_pytorch_ddp=True, block_master=True):
...
def get_parameter_groups_for_encoder(model, weight_decay=1e-5, base_lr=1e-4, text_encoder_lr=5e-5, vision_encoder_lr = 2e-5, vision_head_lr = 1e-4, skip_list=(), get_num_layer=None, get_layer_scale=None, **kwargs):
    parameter_group_names = {}
    parameter_group_vars = {}
    text_scale = text_encoder_lr / base_lr
    vision_scale = vision_encoder_lr / base_lr
    vision_head_scale =  vision_head_lr / base_lr
    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith('text_tower'):
            group_name += "_text_enc"
            this_scale = text_scale
        elif name.startswith('vision_tower'):
            group_name += "_vision_enc"
            this_scale = vision_scale
        elif name.startswith('vision_head'):
            group_name += "_vision_head"
            this_scale = vision_head_scale
        else:
            group_name += "_other"
            this_scale = 1.0
        ...
        if group_name not in parameter_group_names:
            if get_layer_scale is not None:
                scale = get_layer_scale(layer_id) * this_scale
            else:
                scale = default_scale * this_scale
...
def create_optimizer_for_encoder(args, model, get_num_layer=None, get_layer_scale=None, filter_bias_and_bn=True, skip_list=None, **kwargs):
    ...
    parameters = get_parameter_groups_for_encoder(model, weight_decay, args.lr, args.text_encoder_lr, args.vision_encoder_lr, args.vision_head_lr, skip, get_num_layer, get_layer_scale, **kwargs)
    weight_decay = 0.
