...
def load_music_vectors_from_file(file_path):
    if file_path.endswith('.pt'):
        ...
        ids = [item['music_id'] for item in music_tensor_list]
...
def load_gid_by_lang(
    ...
    """
    返回 dict: { lang: set(gid) }
    """
    result = {}

    for lang in langs:
        path = os.path.join(base_dir, f"{prefix}{lang}{suffix}")
        gids = set()
...
@torch.no_grad()
def retrieve_from_music_library_cases(music_library_dir, output_csv_path='LT/recall_results.csv', topk_num=50, user_language_code=1):
    ...
    set_langs = [] #["en", "es", "id", "th", "vi"]
    lang_to_gids = load_gid_by_lang(set_langs, base_dir="/mnt/bn/jiny-ttls-i18n-fr1q/wangxiuqi.0601/data/country_acceptable_ids", min_count=1000)
    id_to_langs = {} #{1: "en", 6: "id", 2: "es", 9:"th", 10:"vi"}
    all_video_embeds = []
    ...
    with open(output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for idx, image_path in enumerate(image_path_list):
            ...
            if user_language_code in id_to_langs:
                lang = id_to_langs[user_language_code]
                final_topk_music_id_ = [x for x in final_topk_music_id if int(x) in lang_to_gids[lang]]
                final_topk_sim_scores = [final_topk_sim_scores[i] for i, x in enumerate(final_topk_music_id) if int(x) in lang_to_gids[lang]]
                final_topk_music_id = final_topk_music_id_
                final_topk_music_id = final_topk_music_id[:50]
                final_topk_sim_scores = final_topk_sim_scores[:50]
...
if __name__ == "__main__":
    ...
    # music_library_dir = os.path.join(args.data_path, "7600w_deploy")
    # music_library_dir = "/mnt/bn/jiny-ttls-i18n-fr1q/lutong/muisc_embs/20260101_1200w"
    # music_library_dir = "/mnt/bn/jiny-ttls-i18n-fr1q/wangxiuqi.0601/checkpoints/video_siglip2_for_music_only_match_huber_loss_delta_1_add_lang/checkpoint-19600/200w_deploy/"
    retrieve_from_music_library_cases(music_library_dir, args.output_csv_path, topk_num=300, user_language_code=args.user_language_code)
