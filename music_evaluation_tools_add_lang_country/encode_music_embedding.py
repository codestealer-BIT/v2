...
from video_clip import VideoSigLIP2ForMusic, VideoSigLIP2ForMusicOnlyMatching,VideoSigLIP2ForMusicOnlyMatchingContrastive, VideoSigLIP2ForMusicOnlyMatchingAddUserInfo, VideoSigLIP2ForMusicNegativeFilteringAddLang, VideoSigLIP2ForMusicNegativeFilteringBMAddLang, VideoSigLIP2ForMusicOnlyMatchingV2
...
def build_model(model_path, pretrained_ckpt_path, args):
    ...
    # model = VideoSigLIP2ForMusicOnlyMatchingContrastive(
    #         model_path,
    #         gpuwise_nce=False,
    #         interpolate=6,
    #         use_frame_mask = True,
...
def calculate_music_vectors(music_tensor_file, model, tokenizer, output_dir, device, music_caption_file_dict, args):
    ...
    for i in tqdm(range(0, len(all_music_content_vector), batch_size)):
        ...
        batch_music_title_vector = all_music_title_vector[i:i+batch_size].to(device) if args.add_title_ue else None

        with torch.no_grad():
            batch_music_ue_vector = model.extract_music_embeds(
                batch_music_content_vector, batch_music_cover_vector, batch_music_attribute_input_ids, batch_music_caption_input_ids,
                music_title_embedding=batch_music_title_vector)
...
def calculate_evalset_music_vectors(music_dict_file, model, tokenizer, output_dir, device, args):
    ...
    for i in tqdm(range(0, len(all_music_content_vector), batch_size)):
        ...
        with torch.no_grad():
            batch_music_ue_vector = model.extract_music_embeds(
                batch_music_content_vector, batch_music_cover_vector, batch_music_attribute_input_ids, batch_music_caption_input_ids, 
                music_title_embedding=batch_music_dist_codes)
            batch_music_ue_vector = batch_music_ue_vector.cpu()
...
def extract_video_embeds_cases(ckpt_path, user_language_code, user_country_code, return_npy=False):
    ...
    for image_path in image_path_list:
        ...
        emb_np = video_embed.squeeze(0).detach().cpu().numpy().astype(np.float32)
        all_embeds.append(emb_np)
        all_item_ids.append(image_name)

    if return_npy:
        print(f"saving npy file to {demo_image_dir}/demo_cases...")
        output_npy_path = os.path.join(demo_image_dir, 'demo_cases/query.npy')
        output_meta_path = os.path.join(demo_image_dir, 'demo_cases/query_meta.csv')
        os.makedirs(os.path.dirname(output_npy_path), exist_ok=True)
        np.save(output_npy_path, np.stack(all_embeds, axis=0))
        with open(output_meta_path, 'w', newline='') as f:
            w = csv.writer(f)
            w.writerow(['idx', 'item_id'])
            for i, item_id in enumerate(all_item_ids):
                w.writerow([i, item_id])
