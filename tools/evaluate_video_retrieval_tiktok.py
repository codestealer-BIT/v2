...
from video_clip import VideoSigLIP2Temporal, VideoSigLIP2ShortLongBLIP, VideoSigLIP2ShortLong
...
def validate_msrvtt(model, dataloader,
    ...
    for idx,data in tqdm.tqdm(enumerate(dataloader)):
        ...
        text_input_ids = data['short_input_ids'].to(device)
    ...
    if output_dir is not None:
        with open(output_dir + "/val_tiktok_log.txt", "a") as f:
...
if __name__ == '__main__':
    ...
    if args.use_frame_mask:
        print("Using frame mask")
    else:
        print("Not using frame mask")
