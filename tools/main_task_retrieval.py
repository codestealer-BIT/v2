...
from video_clip import VideoSigLIP2ShortLong, VideoSigLIP2ShortLongBLIP
...
def get_args(description='Siglip2 on Retrieval Task'):
    ...
    parser.add_argument('--use_frame_mask', action='store_true', help="Whether to use frame mask in evaluation")
    ...
    parser.add_argument("--stage",type=int, default=1, help="which stage model")
...
def init_model(args, device, n_gpu, local_rank):
    if args.stage == 1:
        model = VideoSigLIP2ShortLong(model_path=args.pretrained_config, interpolate=6, use_frame_mask = args.use_frame_mask)
    else:
        model = VideoSigLIP2ShortLongBLIP(model_path=args.pretrained_config, blip_path = args.blip_path, interpolate=6, use_frame_mask = args.use_frame_mask) 
    
    model.load_state_dict(torch.load(args.pretrained_model + '/pytorch_model.bin',map_location='cpu'))
...
def _run_on_single_gpu(model, batch_sequence_output_list, batch_visual_output_list):
    ...
    for idx1, b1 in enumerate(batch_sequence_output_list):
        ...
        for idx2, b2 in enumerate(batch_visual_output_list):
            ...
            b1b2_logits = scores * logit_scale.exp() + logit_bias
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            batch = (t.to(device) for t in batch[:-2]) # to remove base64 frame str and caption str
            text_input_ids, pixel_values, pixel_attention_mask, spatial_shapes,frame_mask = batch

            if not model.use_frame_mask:
                frame_mask = None

            if multi_sentence_:
                # multi-sentences retrieval means: one clip has two or more descriptions.
                b, *_t = pixel_values.shape
                if isinstance(model, VideoSigLIP2ShortLong):
                    sequence_output = model.encode_text(text_input_ids)
                else:
                    _,sequence_output = model.encode_text(text_input_ids)
                ...
                if len(filter_inds) > 0:
                    pixel_values, pixel_attention_mask, spatial_shapes = pixel_values[filter_inds, ...], pixel_attention_mask[filter_inds, ...], spatial_shapes[filter_inds, ...]
                    if isinstance(model, VideoSigLIP2ShortLong):
                        visual_output = model.encode_video(pixel_values,pixel_attention_mask,spatial_shapes, frame_mask)
                    else:
                        _,visual_output = model.encode_video(pixel_values,pixel_attention_mask,spatial_shapes, frame_mask)
        ...
        if n_gpu > 1:
            ...
            bacth_len = len(batch_t_output_splits)
