...
from dataset import VideoHDFSDatasetFeatureExtract, create_feature_extract_dataloader
...
def get_args():
    ...
    parser.add_argument('--fuse_caption', action='store_true', help="Whether to fuse caption")
...
def build_model_runner(args):
    ...
    runner.load_state_dict(torch.load(args.pretrained_path + '/pytorch_model.bin', map_location='cpu'))
...
def main(args):
    ...
    test_dataset = VideoHDFSDatasetFeatureExtract(
        args.hdfs_file_path,
        rank=global_rank,
        ...
        fuse_caption=args.fuse_caption,
    )
    
    loader = create_feature_extract_dataloader(
        test_dataset,
    ...
    runner = accelerator.prepare(runner)
    ...
    pbar = tqdm(total=args.max_steps, desc="Evaluating", disable=not accelerator.is_local_main_process)
    with torch.no_grad():
        try:
            while True:
                # 手动取数据
                ...
                # fused_gathered = accelerator.gather(fused_pooled)
                # item_gathered = accelerator.gather(item_ids)

                local_embeddings.append(fused_pooled.detach().cpu().numpy())
                local_item_ids.append(item_ids.detach().cpu().numpy())
    ...
    rank_embeddings = np.concatenate(local_embeddings, axis=0)
    np.save(os.path.join(args.output_dir, f"embeddings_rank{rank}.npy"), rank_embeddings)

    rank_item_ids = np.concatenate(local_item_ids, axis=0)
    np.save(os.path.join(args.output_dir, f"item_ids_rank{rank}.npy"), rank_item_ids)

    del local_embeddings, local_item_ids, rank_embeddings, rank_item_ids
    ...
    # all_embeddings = accelerator.gather(accelerator.pad_across_processes(torch.cat(local_embeddings), dim=0, pad_index=-1))
    # all_item_ids = accelerator.gather(accelerator.pad_across_processes(torch.cat(local_item_ids), dim=0, pad_index=-1))

    if accelerator.is_main_process:
        print(f"Merging {world_size} rank outputs...")
        all_embeddings = []
        for r in range(world_size):
            rank_embeddings = np.load(os.path.join(args.output_dir, f"embeddings_rank{r}.npy"))
            all_embeddings.append(rank_embeddings)
        all_embeddings = np.concatenate(all_embeddings, axis=0)

        all_item_ids = []
        for r in range(world_size):
            rank_item_ids = np.load(os.path.join(args.output_dir, f"item_ids_rank{r}.npy"))
        ...
        faiss.write_index(index, os.path.join(args.output_dir, "vectors.index"))
        ...
        #     df_chunk.to_parquet(os.path.join(args.output_dir,f"embeddings/fusion_embedding_chunk_{i}.parquet"), index=False)
