...
class VideoHDFSDatasetFusionBLIP(IterableDataset):
    def __init__(
            self, 
            data_path,
            dict_path,
    ...
    def process_text(self, captions, titles, stickers, ocr_texts, asr_texts): 
        ...
        text_encoding = self.blip_tokenizer(captions + "</s>", padding='max_length', truncation=True, max_length=self.max_text_len, return_attention_mask = True, return_tensors="pt")
        ...
        return caption_input_ids, blip_input_ids, blip_attention_mask, pad_title, pad_segment, title_attention_mask
    ...
    def __iter__(self):
        for item_str in self.generate():
            try:
                ...
                caption_input_ids, blip_input_ids, blip_attention_mask, title_input_ids, title_segment_ids, title_attention_mask = self.process_text(captions = video_caption, titles = title, stickers = sticker, ocr_texts = ocr_text, asr_texts = asr_text)
...
if __name__ == "__main__":
    ...
    hdfs_dataset = VideoHDFSDatasetFusionBLIP(data_path=data_path, dict_path = dict_path, shuffle=True, repeat=True, image_query_num = 10, buffer_size=256, max_frame_len=5, tokenizer_dir=tokenizer_dir,blip_tokenizer_dir = blip_tokenizer_dir)
