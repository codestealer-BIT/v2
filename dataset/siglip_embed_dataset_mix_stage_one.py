...
class VideoDatasetMixtureStageOne(IterableDataset):
    """
    ...
    def __init__(self, datasets, weights):
        # 确保数据集和权重数量匹配
        assert len(datasets) == len(weights), "数据集数量和权重数量必须相等"
        # 确保权重总和为1（或接近1，考虑浮点数误差）
        assert abs(sum(weights) - 1.0) < 1e-6, "权重总和必须为1.0"
...
if __name__ == "__main__":
    ...
    internvid_dataset = VideoDatasetInternVid(data_path=data_path, shuffle=True, repeat=True, image_query_num = 10, buffer_size=256, max_frame_len=5, max_short_len = 10, max_text_len = 64,tokenizer_dir=tokenizer_dir,blip_tokenizer_dir = blip_tokenizer_dir)
    ...
    hdfs_dataset = VideoHDFSDatasetShortLong(data_path=data_path, dict_path = dict_path, shuffle=True, repeat=True, image_query_num = 10, buffer_size=256, max_frame_len=5, max_short_len = 20, max_text_len = 30,tokenizer_dir=tokenizer_dir,blip_tokenizer_dir = blip_tokenizer_dir)
    ...
    large_dataset = VideoHDFSDatasetLarge(data_path=data_path, dict_path = dict_path, shuffle=True, repeat=True, image_query_num = 10, buffer_size=256, max_frame_len=5, max_short_len = 30, max_text_len = 30,tokenizer_dir=tokenizer_dir,blip_tokenizer_dir = blip_tokenizer_dir)

    # make mixture
    mix_dataset = VideoDatasetMixtureStageOne(datasets=[internvid_dataset, hdfs_dataset, large_dataset], weights=[0.5, 0.25, 0.25])
