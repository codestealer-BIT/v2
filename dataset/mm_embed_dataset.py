    ...
    def __iter__(self):
        for item_str in self.generate():
            try:
                ...
                video_tensors, frames_mask = self.read_video_frames(video_frames)
                video_title = data_item['text']
                video_caption = data_item['video_caption']

                if self.use_text_feature:
                    # Filter 
                    if video_caption == '':
                        if item_id in CAPTION_DICT:
                            video_caption = CAPTION_DICT[item_id]
                        else:
                            continue
                    text_input_ids, text_segment_ids = self.process_text([video_caption])
                    text_input_masks = np.array(text_input_ids > 0, dtype=np.int32)
                    text_input_ids = torch.LongTensor(text_input_ids)
                    text_segment_ids = torch.LongTensor(text_segment_ids)
                    text_input_masks = torch.LongTensor(text_input_masks)
                else:
                    text_input_ids = None
                    text_segment_ids = None
                    text_input_masks = None
                ...
                # print(video_tensors.shape, frames_mask.shape, music_content_vector.shape, music_title_vector.shape, music_cover_vector.shape)
                music_ue_vector = (music_content_vector + music_title_vector + music_cover_vector) / 3

                res = {
                    'item_ids': str(item_id), 'music_ids': str(music_id), 'frames': video_tensors, 
                    'frames_mask': frames_mask, 'music_ue_vector': music_ue_vector,
                }
...
if __name__ == "__main__":
    ...
    hdfs_dataset = VideoHDFSDataset(data_path, shuffle=True, repeat=True, buffer_size=256, max_frame_len=5, tokenizer_dir=tokenizer_dir, use_text_feature=True)
    ...
    for data in loader:
        print(data['music_ids'], data['text_input_ids'])
        time.sleep(1)
