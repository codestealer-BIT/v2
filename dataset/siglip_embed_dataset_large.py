...
class VideoHDFSDatasetLarge(IterableDataset):
    def __init__(
            self, 
            data_path,
            dict_path = None,
        ...
        self.filelist_cache = filelist_cache
        # if isinstance(data_path, str):
        #     data_path = [data_path]

        # #copied from jinghan
        # if self.filelist_cache is not None:
        #     if self.rank == 0:
        #         cache_fp = os.path.join(self.filelist_cache, f'rank_{self.rank}.jsonl')
        #         if not os.path.exists(self.filelist_cache):
        #             os.makedirs(self.filelist_cache)

        #         tmp_cache_fp = cache_fp + '.tmp'      
        #         self.files = self.multi_thread_get_files(data_path)
        #         with jsonlines.open(tmp_cache_fp, 'w') as writer:
        #             writer.write_all(self.files)
        #         os.rename(tmp_cache_fp, cache_fp)
        #         print(f'[DATA]--Rank 0 cached filelist to {cache_fp}')

        #         for i in range(1, self.world_size):
        #             previous_cache = os.path.join(self.filelist_cache, f'rank_{i}.jsonl')
        #             if os.path.exists(previous_cache):
        #                 os.remove(previous_cache)
        #                 print(f'[DATA]--Rank 0 removed the previous cache of rank {i}: {previous_cache}')
        #     else:
        #         source_cache = os.path.join(self.filelist_cache, 'rank_0.jsonl')
        #         dst_cache = os.path.join(self.filelist_cache, f'rank_{self.rank}.jsonl')
        #         while (not os.path.exists(source_cache)) or os.path.exists(dst_cache):
        #             time.sleep(10)
        #             print(f'[DATA]--Rank {self.rank} wait for rank 0')
        #         time.sleep(10) # for rank 0 to finish check previous caches, ugly solution
        #         shutil.copyfile(source_cache, dst_cache)
        #         with jsonlines.open(dst_cache, 'r') as reader:
        #             self.files = list(reader)
        #         print(f'[DATA]--Rank {self.rank} copied filelist from {source_cache} to {dst_cache}')
        # else:
        #     print(f'[DATA]--Rank {self.rank} multi thread get files')
        #     self.files = self.multi_thread_get_files(data_path)
    ...
    def get_caption_dict(self, dict_path):
        ...
        self.caption_dict = {
            "short_caption": os.path.join(dict_path, "short_caption/pretrain_data_0911_100m_data"),
            "long_caption": os.path.join(dict_path, "long_caption/pretrain_data_0911_100m_data"),
            "fusion_caption": os.path.join(dict_path, "fusion_caption/pretrain_data_0911_100m_data")
        }
...
if __name__ == "__main__":
    ...
    hdfs_dataset = VideoHDFSDatasetLarge(data_path=data_path, dict_path = dict_path, shuffle=True, repeat=True, image_query_num = 10, buffer_size=256, max_frame_len=5, tokenizer_dir=tokenizer_dir,blip_tokenizer_dir = blip_tokenizer_dir)
