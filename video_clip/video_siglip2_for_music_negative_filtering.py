...
class VideoSigLIP2ForMusicNegativeFiltering(torch.nn.Module):
    def __init__(self,
                 model_path,
        ...
        super().__init__()
        # module init
        print("start init VideoSigLIP2ForMusicNegativeFiltering")
        ...
        if historical_model_path is not None:
            # 加载历史模型（创建一个同结构但不再递归加载历史模型的实例）
            # 注意：当创建历史模型实例时，传入 historical_model_path=None 以避免递归加载
            hist_model = VideoSigLIP2ForMusicNegativeFiltering(
                ...
                historical_model_path=None  # 防止递归
            )

            # 尝试加载检查点（支持直接 state_dict 或 {'state_dict': ...} 等常见格式）
            try:
                ckpt = torch.load(historical_model_path, map_location='cpu')
                if isinstance(ckpt, dict) and 'state_dict' in ckpt:
                    state_dict = ckpt['state_dict']
                else:
                    state_dict = ckpt
                # 尝试直接加载（容错：allow_missing / strict=False）
                hist_model.load_state_dict(state_dict, strict=False)
            except Exception as e:
                # 若直接加载失败，再尝试按常见 key 处理
                print("historic load by key")

                try:
                    # 如果 ckpt 包含 'model' 或 'model_state_dict'
                    if isinstance(ckpt, dict):
                        for key in ['model', 'model_state_dict', 'state']:
                            if key in ckpt:
                                hist_model.load_state_dict(ckpt[key], strict=False)
                                break
                except Exception as e2:
                    # 若仍然失败，raise 便于用户发现
                    print("error historic load")
                    raise RuntimeError(f"加载历史模型失败: {e}; fallback error: {e2}")
            print("finish historic load")
            # 冻结并设为 eval
            hist_model.eval()
            for p in hist_model.parameters():
                p.requires_grad = False

            self.historical_model = hist_model
        # 初始化权重
        self._initialize_weights()
    ...
    def calc_siglip_loss(self,
        ...
        print(f"保留下来的正例数: {kept_pos_count} / {N}")
