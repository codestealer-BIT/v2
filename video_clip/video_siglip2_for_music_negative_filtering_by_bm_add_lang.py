...
class VideoMusicLangMatchingLoss(nn.Module):
    def __init__(self, cls_dim: int = 128, compress_embedding_size: int = 128):
        super(VideoMusicLangMatchingLoss, self).__init__()
        self.side_logit_scale = nn.Parameter(torch.randn([1]))
        self.side_logit_bias = nn.Parameter(torch.randn([1]))
        self.side_video_proj = nn.Linear(cls_dim, compress_embedding_size)
        self.side_music_proj = nn.Linear(compress_embedding_size, compress_embedding_size)
        self.side_video_lang_head = nn.Linear(cls_dim, 32)
        self.side_video_country_head = nn.Linear(cls_dim, 32)
        self.side_music_lang_head = nn.Linear(compress_embedding_size, 32)
        self.side_lang_cls = nn.Sequential(nn.Linear(2, 32), nn.GELU(), nn.Linear(32, 1))
        self.side_country_cls = nn.Sequential(nn.Linear(2, 32), nn.GELU(), nn.Linear(32, 1))


        self.video_lang_to_music_lang_multi = video_lang_to_music_lang_multi
        self.country_to_music_lang_multi = country_to_music_lang_multi

    def forward(self,
            video_embeds: torch.Tensor,
            music_embeds: torch.Tensor,
            video_lang_indices: torch.Tensor,
            video_country_indices: torch.Tensor,
            music_lang_indices: torch.Tensor,
        ):
        # base_diag -> a prior that fused pair’s similarity & anchors the classifier in the actual retrieval-space similarity, not just in head-level language features.
        v_proj = F.normalize(self.side_video_proj(video_embeds), dim=-1)
        m_proj = F.normalize(self.side_music_proj(music_embeds), dim=-1)
        side_logits = torch.mm(m_proj, v_proj.t())
        side_logits = side_logits * self.side_logit_scale.exp() + self.side_logit_bias
        base_diag = torch.diag(side_logits)

        v_lang = F.normalize(self.side_video_lang_head(video_embeds), dim=-1)
        v_country = F.normalize(self.side_video_country_head(video_embeds), dim=-1)
        m_lang = F.normalize(self.side_music_lang_head(music_embeds), dim=-1)
        lang_match = torch.mm(m_lang, v_lang.t())
        country_match = torch.mm(m_lang, v_country.t())

        lang_pair_feat = torch.stack([base_diag, torch.diag(lang_match)], dim=-1)
        country_pair_feat = torch.stack([base_diag, torch.diag(country_match)], dim=-1)
        lang_logits = self.side_lang_cls(lang_pair_feat).squeeze(-1)
        country_logits = self.side_country_cls(country_pair_feat).squeeze(-1)

        # multi-label mappings only, excluding non_vocal from loss
        non_vocal_idx = music_lang2idx.get('non_vocal', 4)
        lang_label_probs = []
        country_label_probs = []
        valid_lang = []
        valid_country = []
        for i in range(music_lang_indices.size(0)):
            vl_idx = int(video_lang_indices[i].item())
            vc_idx = int(video_country_indices[i].item())
            m_idx = int(music_lang_indices[i].item())
            allowed_vl = self.video_lang_to_music_lang_multi.get(vl_idx, set())
            allowed_vc = self.country_to_music_lang_multi.get(vc_idx, set())
            is_non_vocal = (m_idx == non_vocal_idx)
            prob_vl = 1.0 if (m_idx in allowed_vl) else 0.0
            prob_vc = 1.0 if (m_idx in allowed_vc) else 0.0
            valid_lang.append((len(allowed_vl) > 0) and (vl_idx != 0) and (not is_non_vocal)) # None & non_vocal excluded
            valid_country.append((len(allowed_vc) > 0) and (vc_idx != 0) and (not is_non_vocal))  # None & non_vocal excluded
            lang_label_probs.append(prob_vl)
            country_label_probs.append(prob_vc)
        lang_labels = torch.tensor(lang_label_probs, dtype=torch.float, device=music_embeds.device)
        country_labels = torch.tensor(country_label_probs, dtype=torch.float, device=music_embeds.device)
        valid_lang_mask = torch.tensor(valid_lang, dtype=torch.bool, device=music_embeds.device)
        valid_country_mask = torch.tensor(valid_country, dtype=torch.bool, device=music_embeds.device)

        lang_loss_vec = F.binary_cross_entropy_with_logits(lang_logits, lang_labels, reduction='none')
        country_loss_vec = F.binary_cross_entropy_with_logits(country_logits, country_labels, reduction='none')
        lang_side_loss = lang_loss_vec[valid_lang_mask].mean() if valid_lang_mask.any() else torch.tensor(0.0, device=music_embeds.device)
        country_side_loss = country_loss_vec[valid_country_mask].mean() if valid_country_mask.any() else torch.tensor(0.0, device=music_embeds.device)


        with torch.no_grad():
            N = lang_match.size(0)
            diag = torch.eye(N, dtype=torch.bool, device=lang_match.device)
            off = ~diag
            self.latest_stats = {
                "side_base_diag_mean": base_diag.mean().item(),
                "side_lang_diag_mean": lang_match[diag].mean().item(),
                "side_lang_offdiag_mean": lang_match[off].mean().item(),
                "side_country_diag_mean": country_match[diag].mean().item(),
                "side_country_offdiag_mean": country_match[off].mean().item(),

            }

        return lang_side_loss, country_side_loss
...
class VideoSigLIP2ForMusicNegativeFilteringBMAddLang(torch.nn.Module):
    def __init__(self,
                 model_path,
                 ...
                 enable_lang_loss: bool = False,
                 **kwargs):
        super().__init__()
        # module init
        print("start init VideoSigLIP2ForMusicNegativeFilteringBMAddLang")
        print("historical_model_path", historical_model_path)
        print("historical_score_threshold", historical_score_threshold)
        print("VideoSigLIP2ForMusicNegativeFilteringBMAddLang user_info_fusion_method", user_info_fusion_method)
        print("enable_lang_loss", enable_lang_loss)
        ...
        self.enable_lang_loss = enable_lang_loss
        if self.enable_lang_loss:
            self.cal_video_music_lang_matching_loss = VideoMusicLangMatchingLoss(cls_dim=self.compress_embedding_size, compress_embedding_size=self.compress_embedding_size)
    ...
    def init_modules(self, model_path):
        ...
        if self.enable_lang_loss:
            self.cal_video_music_lang_matching_loss.side_logit_scale.data.fill_(logit_scale_init)
            self.cal_video_music_lang_matching_loss.side_logit_bias.data.fill_(-10.0)
    ...
    def forward(
        ...
        # side task: language/country match from fused embeddings
        if self.enable_lang_loss:
            ...
            lang_side_loss, country_side_loss = self.cal_video_music_lang_matching_loss(
                cls_visual_embedding, music_embedding, lang_indices, country_indices, music_lang_indices
            )
            ret_dict['lang_side_task_loss'] = lang_side_loss
            ret_dict['lang_country_side_task_loss'] = country_side_loss
            stats = getattr(self.cal_video_music_lang_matching_loss, 'latest_stats', {})
            ret_dict.update(stats)
