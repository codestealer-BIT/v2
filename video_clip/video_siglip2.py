...
from transformers import AutoConfig, Siglip2VisionModel, Siglip2TextModel
...
class VideoSigLIP2(torch.nn.Module):
    def __init__(self,
                 model_path,
    ...
    def forward(
        ...
        text_attention_masks = samples['text_attention_masks']
        
        ret_dict = dict()
        # text
        t_embs = self.encode_text(input_ids=text_input_ids, attention_mask = text_attention_masks)
        ...
        if self.queue_size > 0:
            with torch.no_grad():
                ...
                t_embs_m = model_ema.encode_text(input_ids=text_input_ids, attention_mask = text_attention_masks)
...
if __name__ == "__main__":
    ...
    videosig = VideoSigLIP2(model_path = model_path, gpuwise_nce = False)
    ...
    text_inputs = tokenizer(text, padding="max_length", max_length = 64, return_tensors="pt", return_attention_mask = True)
    ...
    samples['text_attention_masks'] = text_attention_masks.to("cuda")
