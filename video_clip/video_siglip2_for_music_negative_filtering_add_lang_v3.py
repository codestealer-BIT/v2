...
class MusicFeatureFusion(nn.Module):
    ...
    def __init__(self, high_input_dim=768, low_input_dim=128, hidden_dim=512, output_dim=128):
        ...
        # ugc, pugc, pgc embedding
        self.pgc_embedding = nn.Embedding(3, 16) ## conditional signal for attention weight
        self.attn_linear = nn.Linear(hidden_dim * 4 + 16, 4)
    ...
    def forward(self, music_content, music_cover, music_attribute, music_caption, music_dist_code=None):
        """
        前向传播过程
        ...
        pgc_emb = self.pgc_embedding(torch.as_tensor(music_dist_code, dtype=torch.long, device=content_feat.device))
        concat_feat = torch.cat([content_feat, cover_feat, attribute_feat, caption_feat, pgc_emb], dim=1) # (batch_size, 4*hidden_dim)
        
        # 计算注意力权重
        attn_logits = self.attn_linear(concat_feat)  # (batch_size, 4)
        attn_weights = F.softmax(attn_logits, dim=1).unsqueeze(-1)   # (batch_size, 4, 1)    
        
        # 应用注意力权重
        weighted_content = content_feat * attn_weights[:, 0, :]
        weighted_cover = cover_feat * attn_weights[:, 1, :]
        weighted_attribute = attribute_feat * attn_weights[:, 2, :]
        weighted_caption = caption_feat * attn_weights[:, 3, :]
        
        # 特征融合 - 采用元素级相加与mean pooling融合的组合策略
        sum_fused = weighted_content + weighted_cover + weighted_attribute + weighted_caption
        mean_fused = torch.mean(torch.stack([weighted_content, weighted_cover, weighted_attribute, weighted_caption]), dim=0)
        fused = sum_fused + mean_fused
...
class VideoMusicMatchingLoss(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256, num_category=5):
        """
        >= 7 : high quality
...
class UserContextAdapter(nn.Module):
    def __init__(self,
                 ...
                 hidden_dim: int = 128,
                 language_add_constant: float = 0.0):
        super().__init__()
        self.cls_dim = cls_dim
        self.language_add_constant = language_add_constant
        # embedding tables
        ...
        print(f"UserContextAdapter language_add_constant: {self.language_add_constant}")
        self.gate_proj = nn.Sequential(
            nn.Linear(hidden_dim, 1),
            AddConstant(self.language_add_constant),
            nn.Sigmoid()
        )
        ...
        self.input_layer_norm = nn.LayerNorm(cls_dim, eps=1e-6)        # init small weights
        self._init_weights()
    ...
    def forward(self, cls_visual_embedding: torch.Tensor, lang_indices: torch.Tensor, country_indices: torch.Tensor):
        ...
        if self.training:
            lang_mask = (torch.rand(B, 1, device=device) > 0.1).float() # 10% 概率丢弃 Lang
            country_mask = (torch.rand(B, 1, device=device) > 0.1).float() # 10% 概率丢弃 Country
            lang_emb = lang_emb * lang_mask
            country_emb = country_emb * country_mask

        ctx = torch.cat([lang_emb, country_emb], dim=-1)
        ctx = self.context_mlp(ctx)
        gate = self.gate_proj(ctx)  # (B, cls_dim) in (0,1)
        user_res = self.res_proj(ctx)  # (B, cls_dim) in (-1,1)
        with torch.no_grad():
            self.latest_gate_mean = gate.mean().item()
        out =  self.input_layer_norm(cls_visual_embedding) + user_res * gate

        return out


class VideoSigLIP2ForMusicNegativeFilteringAddLang(torch.nn.Module):
    def __init__(self,
                 model_path,
                 ...
                 language_add_constant: float = 0.0,
                 filter_same_meta_prob: float = 0.0,
                 **kwargs):
        super().__init__()
        # module init
        print("start init VideoSigLIP2ForMusicNegativeFilteringAddLang")
        print("historical_model_path", historical_model_path)
        print("historical_score_threshold", historical_score_threshold)
        print(f"filter_same_meta_prob: {filter_same_meta_prob}")
        ...
        self.gpuwise_nce = gpuwise_nce
        self.mean_loss = mean_loss
        self.use_frame_mask = use_frame_mask
        self.filter_same_meta_prob = filter_same_meta_prob
        ...
        self.cal_video_music_matching_loss = VideoMusicMatchingLoss(input_dim=self.compress_embedding_size, num_category=5)
        ...
        # 初始化权重
        self._initialize_weights()
        self.user_context_adapter = UserContextAdapter(cls_dim=cls_dim, lang_emb_dim=32, country_emb_dim=32, language_add_constant=language_add_constant)
        
        self.historical_model_loaded = False
        if historical_model_path is not None:
            print("historical_model_path is not None")
            hist_model = VideoSigLIP2ForMusic(
    ...
    def calc_siglip_loss(self,
        ...
        user_lang_indices: torch.Tensor = None,
        user_country_indices: torch.Tensor = None,
    ):
        # normalized features
        ...
        if user_lang_indices is not None and user_country_indices is not None:
            # 过滤掉与用户语言或国家相同的负例
            lang_match = (user_lang_indices.unsqueeze(1) == user_lang_indices.unsqueeze(0))
            country_match = (user_country_indices.unsqueeze(1) == user_country_indices.unsqueeze(0))
            is_same_meta = lang_match | country_match


            # 排除对角线
            is_off_diagonal = ~torch.eye(logits_per_text.size(0), dtype=torch.bool, device=logits_per_text.device)
            
            # 候选集：原本是负例(非对角线) 且 (User属性相同)
            candidate_mask = is_same_meta & is_off_diagonal
            total_same_meta_negatives = candidate_mask.sum().item() 
            # print("total_same_meta_negatives", total_same_meta_negatives)
            if self.filter_same_meta_prob > 0:
                if self.filter_same_meta_prob >= 1.0:
                    filter_mask = candidate_mask
                else:
                    rand_mask = torch.rand_like(logits_per_text) < self.filter_same_meta_prob
                    filter_mask = candidate_mask & rand_mask
                
                exclude_mask[filter_mask] = True
                meta_filtered_count = filter_mask.sum().item()
                # print("meta_filtered_count", meta_filtered_count)
        with torch.no_grad():
        ...
        if total_same_meta_negatives > 0:
            same_meta_filtered_ratio = meta_filtered_count / total_negatives
        # print("same_meta_filtered_ratio", same_meta_filtered_ratio)
        stats_tensor = torch.tensor([filtered_ratio, same_meta_filtered_ratio], device=logits_per_text.device)
        if is_dist_avail_and_initialized():
            dist.all_reduce(stats_tensor, op=dist.ReduceOp.AVG)
        filtered_ratio = stats_tensor[0].item()
        same_meta_filtered_ratio = stats_tensor[1].item()
        ...
        return loss, {
            ...
            "same_meta_total": total_same_meta_negatives,
            "same_meta_filtered_count": meta_filtered_count,
            "same_meta_filtered_ratio": same_meta_filtered_ratio,
        }
    ...
    def forward(
        ...
        for i_b in range(len(video_music_match_score)):
            ...
            video_music_match_labels.append(score -1)
        ...
        with torch.no_grad():
            ...
            music_attribute_embedding = self.encode_text(input_ids=music_attribute_input_ids, attention_mask=music_attribute_attention_mask)
            music_caption_embedding = self.encode_text(input_ids=music_caption_input_ids, attention_mask=music_caption_attention_mask)

        music_languages = samples["music_lang_code"]
        music_lang_indices = torch.tensor(
            music_languages,
            dtype=torch.long, device=pixel_values.device
        )

        music_embedding = self.music_tower(music_content_embedding, music_cover_embedding, music_attribute_embedding, music_caption_embedding, samples['music_dist_code'])
        ...
        if self.user_context_adapter.latest_gate_mean is not None:
            ret_dict['user_gate_mean'] = self.user_context_adapter.latest_gate_mean
        ...
        if self.gpuwise_nce:
            ...
            def gather_indices(ind):
                gathered = [torch.zeros_like(ind) for _ in range(world_size)]
                dist.all_gather(gathered, ind)
                return torch.cat(gathered, dim=0)

            user_lang_all = gather_indices(lang_indices)
            user_country_all = gather_indices(country_indices)
        else:
            ...
            user_lang_all = lang_indices
            user_country_all = country_indices
        ...
        video_music_siglip_loss, neg_stat = self.calc_siglip_loss(
            ...
            user_lang_indices=user_lang_all,
            user_country_indices=user_country_all,
        )
        ...
        ret_dict.update({
            ...
            "same_meta_total": neg_stat["same_meta_total"],
            "same_meta_filtered_ratio": neg_stat["same_meta_filtered_ratio"],
            "same_meta_filtered_count": neg_stat["same_meta_filtered_count"]
        })
    ...
    @torch.no_grad()
    def extract_music_embeds(self, music_content_embedding, music_cover_embedding, music_attribute_input_ids, music_caption_input_ids, music_dist_code=None):
        ...
        music_embedding = self.music_tower(music_content_embedding, music_cover_embedding, music_attribute_embedding, music_caption_embedding, music_dist_code)
