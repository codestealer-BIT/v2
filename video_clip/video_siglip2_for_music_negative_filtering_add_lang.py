...
class UserContextAdapter(nn.Module):
    def __init__(self,
        ...
        if method == 'gate':
            ...
            self.input_layer_norm = nn.LayerNorm(cls_dim, eps=1e-6)
        elif method == 'film':
    ...
    def forward(self, cls_visual_embedding: torch.Tensor, lang_indices: torch.Tensor, country_indices: torch.Tensor):
        ...
        if self.method == 'gate':
            ...
            out =  self.input_layer_norm(cls_visual_embedding) * (1 - gate) + user_res * gate
...
class VideoSigLIP2ForMusicNegativeFilteringAddLang(torch.nn.Module):
    def __init__(self,
                 ...
                 user_info_fusion_method: str= "residual",
                 language_add_constant: float = 0.0,
                 **kwargs):
        super().__init__()
        # module init
        ...
        print("VideoSigLIP2ForMusicNegativeFilteringAddLang user_info_fusion_method", user_info_fusion_method)
        ...
        # 初始化权重
        self._initialize_weights()
        self.user_context_adapter = UserContextAdapter(cls_dim=cls_dim, lang_emb_dim=32, country_emb_dim=32, method=user_info_fusion_method, language_add_constant=language_add_constant)
    ...
    def forward(
        ...
        if self.user_context_adapter.method == 'gate' and self.user_context_adapter.latest_gate_mean is not None:
