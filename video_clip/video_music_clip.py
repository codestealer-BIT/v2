...
import pickle
import sys
sys.path.append('/mlx_devbox/users/yexiaoyu.0711/playground/MMPretrain')
...
class VideoMusicCLIP(torch.nn.Module):
    def __init__(self,
                 config = None,
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, music_feat, music_item_ids):
    ...
    def forward(
        ...
        if self.queue_size > 0:
            ...
            self._dequeue_and_enqueue(music_embedding, music_item_ids)
        ...
        valid_idx = None
        visual_nce_loss = self.calc_nce_loss(v_emb=cls_visual_embedding_all, t_emb=music_embedding_all, valid_idx=valid_idx, use_double_loss=False)
        ret_dict['visual_nce_loss'] = visual_nce_loss * self.main_loss_weight
...
if __name__ == "__main__":
    ...
    cfg.update_cfg('/mlx_devbox/users/yexiaoyu.0711/playground/MMPretrain/experiments/config_finetune_mmembed_v3.yaml')

    print(cfg)
    model = VideoMusicCLIP(cfg, gpuwise_nce=False)
    checkpoint_path = '/mlx_devbox/users/yexiaoyu.0711/playground/MMPretrain/models/model_state_epoch_170000.th'
    state_dict = torch.load(checkpoint_path, map_location='cpu')
    load_res = model.load_state_dict(state_dict, strict=False)

    video_input = torch.randn(2, 8, 3, 224, 224)
    video_mask = torch.ones(2, 8)
    music_input = torch.randn(2, 128)
    
    res = model.extract_video_embeds(video_input, video_mask)
    print(res.shape)
