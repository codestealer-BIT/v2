...
class OrdinalHead(nn.Module):
    def __init__(self, num_classes=5):
        ...
        # 1️⃣ cosine → score
        self.score_head = nn.Linear(1, 1)
        nn.init.constant_(self.score_head.weight, 1.0)
        nn.init.zeros_(self.score_head.bias)

        # 2️⃣ K-1 个可学习阈值（raw → 单调）
        self.raw_thresholds = nn.Parameter(
            torch.zeros(num_classes - 1)
        )

    def forward(self, cosine_sim):
        """
        cosine_sim: [B]
        return:
            logits: [B, K-1]   (for BCEWithLogits)
            thresholds: [K-1]  (for debug / monitor)
        """
        # score: [B]
        score = self.score_head(cosine_sim.unsqueeze(-1)).squeeze(-1)

        # 保证 threshold 单调递增
        thresholds = torch.cumsum(
            F.softplus(self.raw_thresholds), dim=0
        )

        # logits[b, k] = score[b] - threshold[k]
        logits = score.unsqueeze(-1) - thresholds.unsqueeze(0)

        return logits, thresholds

class VideoSigLIP2ForMusicOnlyMatchingV3(torch.nn.Module):
    def __init__(self,
                 model_path,
        ...
        self.ordinal_head = OrdinalHead(num_classes=5)
    ...
    def ordinal_predict(self, logits):
    ...
    def calc_regression_loss(self, image_embeds, music_embeds, label):
        ...
        logits, thresholds = self.ordinal_head(cosine_sim)

        # 构造 ordinal target
        # target[b, k] = 1(label >= k+2)
        target = (label.unsqueeze(-1) > torch.arange(
            1, logits.size(1) + 1, device=label.device
        )).float()
        ...
        weights = self.label_weights[label.long() - 1].unsqueeze(-1)
        loss = (loss * weights).mean()

        return loss #, self.ordinal_predict(logits)
    ...
    def forward(
        ...
        video_music_matching_loss = self.calc_regression_loss(cls_visual_embedding, music_embedding, video_music_match_labels)
