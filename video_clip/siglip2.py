...
class Siglip2VisionEmbeddings(nn.Module):
    def __init__(self, config: Siglip2VisionConfig):
        ...
        self.cls_embedding = nn.Embedding(1, self.embed_dim)
    ...
    def forward(self, pixel_values: torch.FloatTensor, spatial_shapes: torch.LongTensor) -> torch.Tensor:
        ...
        # B*F, max_num_patches, hidden_size

        # B*F, 1, hidden_size
        cls_tokens = self.cls_token.expand(pixel_values.shape[0], -1, -1)
        
        # copied from transformer/vit
        # B*F, max_num_patches+1, hidden_size
        ...
        resized_pos = self.resize_positional_embeddings(
            positional_embeddings, spatial_shapes, max_length=pixel_values.shape[1] #max_num_patches
        )
        
        # B*F, max_num_patches+1, hidden_size
        resized_positional_embeddings = torch.cat((self.cls_embedding.weight.expand(pixel_values.shape[0], -1, -1), resized_pos), dim=1)
...
class Siglip2VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        #hidden_states.shape = B*F,(256+1),768
        ...
        #last_hidden_states.shape = B,F,768
        last_hidden_state = self.post_layernorm(hidden_state)[:,0,:].view(batch_size,num_frames,-1).contiguous() #取出所有的cls_token

        
        # #pooler_output.shape = B,1,768
...
class Siglip2MultiheadAttentionPoolingHead(nn.Module):
    ...
    def forward(self, hidden_state: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        #probe.shape batch_size, 1, hidden_size

        hidden_state = self.attention(probe, hidden_state, hidden_state, attn_mask=None)[0]
