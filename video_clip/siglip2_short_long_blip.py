...
class Siglip2TextEmbeddings(nn.Module):
    def __init__(self, config: Siglip2TextConfig, interpolate = 4):
        super().__init__()
        embed_dim = config.hidden_size
...
class Siglip2MultiheadAttentionPoolingHead(nn.Module):
    ...
    def forward(self, hidden_state: torch.Tensor, frame_mask: Optional[torch.Tensor] = None, pixel_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        hidden_state = hidden_state.view(batch_size * num_frame, -1, hidden_state.shape[-1]).contiguous()

        probe_1 = self.probe_n.repeat(batch_size * num_frame, 1, 1)

        if pixel_mask is not None:
            pixel_mask = pixel_mask.view(batch_size * num_frame, -1).contiguous()
        ...
        hidden_state = hidden_state.contiguous().view(batch_size, -1, hidden_state.shape[-1])
        ...
        if frame_mask is not None:
            ...
            frame_mask = frame_mask.repeat(1, self.num_heads, 1, 1)
            frame_mask = frame_mask.view(-1, target_len, source_len)

        pooler_output = self.attention_2(probe_2, hidden_state, hidden_state, attn_mask=frame_mask)[0]
...
class Siglip2FusionPoolingHead(nn.Module):
    ...
    def forward(self, hidden_state: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask.view(-1, target_len, source_len)

        hidden_state = self.attention_2(probe_2, hidden_state, hidden_state, attn_mask=attention_mask)[0]
...
class Siglip2FusionHead(nn.Module):
    def __init__(self, config:Siglip2VisionConfig, max_segments:int = 4, layers: int = 3):
        super().__init__()
        self.layers = layers
        embed_dim = config.hidden_size
    ...
    def forward(self, vision_embed: torch.Tensor, title_embed: torch.Tensor,vision_attn_mask=None, title_attn_mask = None, segment_ids = None):
        #enter point: vision_embed B,256, 768, title_embed B, 256, 768
        batch_size, seq_len, hidden_size = vision_embed.shape
        if segment_ids is not None:
            title_embed = title_embed + self.segment_embedding(segment_ids)
