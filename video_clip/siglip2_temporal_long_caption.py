...
class Siglip2TextEmbeddings(nn.Module):
    def __init__(self, config: Siglip2TextConfig, max_position_embeddings = 196, keep_len = 20):
        super().__init__()
        embed_dim = config.hidden_size
        self.max_position_embeddings=max_position_embeddings
        self.token_embedding = nn.Embedding(config.vocab_size, embed_dim)
        self.position_embedding_o = nn.Embedding(max_position_embeddings, embed_dim)
        ...
        default_flax_embed_init(self.position_embedding_o.weight)

        self.mask1 = torch.zeros([max_position_embeddings, 1])
        self.mask1[:keep_len, :] = 1
        self.mask2 = torch.zeros([max_position_embeddings, 1])
    ...
    def forward(
        ...
        position_embedding = self.position_embedding_o(position_ids)
        position_embedding_n = self.position_embedding_n(position_ids)

        
        embeddings = inputs_embeds + (position_embedding * self.mask1.to(inputs_embeds.device) + position_embedding_n * self.mask2.to(inputs_embeds.device))
...
class Siglip2MultiheadAttentionPoolingHead(nn.Module):
    ...
    def __init__(self, config: Siglip2VisionConfig,probe_size: int = 16, num_frames: int = 8, num_head_layers = 3):
        super().__init__()
    ...
    def forward(self, hidden_state: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        #hidden.shape B*F,16, hidden_size
        ...
        # Last_hidden_state.shape B,F*16,hidden_size
        hidden_state = hidden_state.reshape(batch_frame//self.num_frames, -1, hidden_state.shape[-1]).contiguous()

        # Add frame positional embedding B,F*16,hidden_size
