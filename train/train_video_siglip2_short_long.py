...
from video_clip import VideoSigLIP2ShortLong
...
def get_args():
    ...
    parser.add_argument('--pretrained_path',default=None, type=str, help='the pretrained model of stage one model')
    parser.add_argument('--blip_path',default="", type=str, help='the blip model path')
    parser.add_argument('--load_vision',action='store_true', help='whether to load pretrained vision model weight from model_path')
    parser.add_argument('--load_text',action='store_true', help='whether to load pretrained text model weight from model_path')
    ...
    parser.add_argument('--hdfs_dict_path', default='/mnt/bn/jiny-ttls-i18n-fr1q/final_tiktok_data_for_small_model', type=str, help="The caption path")
...
def build_model_runner(args, queue_size):
    ...
    runner = VideoSigLIP2ShortLong(
        model_path,
        gpuwise_nce=True,
        queue_size=queue_size,
        load_text=args.load_text,
...
def main(args):
    ...
    train_dataset = VideoDatasetMixtureStageOne(datasets = [hdfs_dataset, large_dataset, internvid_dataset],weights = [0.1, 0.6, 0.3])
    ...
    for epoch in range(first_epoch, args.epochs):
        train_stats = train_one_epoch_for_video_siglip_stage_one(
            runner, 
            model_ema,
            accelerator,
            args.model_dtype,
