...
from video_clip import VideoMusicCLIP, VideoMusicTextCLIP, VideLocalServiceCLIP
...
from trainer_misc import (
    ...
    create_optimizer,
    train_one_epoch_for_video_clip,
    constant_scheduler,
    cosine_scheduler,
)
...
def get_args():
    ...
    parser.add_argument('--model_name', default='video_music_clip', choices=['video_music_clip', 'video_music_text_clip', 'video_local_service'], type=str)
    ...
    parser.add_argument('--use_text_feature', action='store_true', help="Whether to use the caption for training")
...
def build_model_runner(args, queue_size):
    ...
    if model_name == 'video_music_clip':
        runner = VideoMusicCLIP(
            config=cfg,
            gpuwise_nce=False,
            queue_size=queue_size,
        )
    elif model_name == 'video_local_service':
        runner = VideLocalServiceCLIP(
            config=cfg,
            gpuwise_nce=False,
        )
    else:
        assert args.model_name == 'video_music_text_clip'
        runner = VideoMusicTextCLIP(
            config=cfg,
            gpuwise_nce=False,
            queue_size=queue_size,
        )
...
def main(args):
    ...
    if args.model_name == 'video_local_service':
        train_dataset = TTLSHDFSDataset(
            anno_file_path,
            rank=global_rank,
            ...
            tokenizer_dir=cfg.network.tokenizer_dir,
            use_text_feature=True,
        )

        train_dataloader = create_ttls_dataloader(
            train_dataset,
        ...
        train_dataset = VideoHDFSDataset(
            anno_file_path,
            rank=global_rank,
            world_size=accelerator.num_processes,
            shuffle=True,
            ...
            buffer_size=256,
            max_frame_len=args.max_frames,
            tokenizer_dir=cfg.network.tokenizer_dir if args.use_text_feature else None,
    ...
    optimizer = create_optimizer(args, runner)
    ...
    for epoch in range(first_epoch, args.epochs):
        train_stats = train_one_epoch_for_video_clip(
            runner, 
            model_ema,
            accelerator,
            args.model_dtype,
